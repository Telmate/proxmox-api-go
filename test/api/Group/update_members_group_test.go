...
func Test_Group_Update(t *testing.T) {
	groupName := pveSDK.GroupName("Test_Group_Update")
	users := []pveSDK.UserID{
		{Name: "Test_Group_Update_1", Realm: "pve"},
		{Name: "Test_Group_Update_2", Realm: "pve"},
	}
	...
	tests := []struct {
		...
		{name: `Create group`,
			test: func(t *testing.T) {
				require.NoError(t, c.Group.Create(ctx, pveSDK.ConfigGroup{
					Name:    groupName,
					Members: &users,
		...
		{name: `Update group`,
			test: func(t *testing.T) {
				require.NoError(t, c.Group.Update(ctx, pveSDK.ConfigGroup{
					Name:    groupName,
