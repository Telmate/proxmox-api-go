...
func Test_Group_Read(t *testing.T) {
	groupName := pveSDK.GroupName("Test_Group_Read")
	users := []pveSDK.UserID{
		{Name: "Test_Group_Read_1", Realm: "pve"},
		{Name: "Test_Group_Read_2", Realm: "pve"},
	}
	...
	tests := []struct {
		...
		{name: `Read group`,
			test: func(t *testing.T) {
				...
				require.Equal(t, group.Name, raw.GetName())
				require.Equal(t, *group.Comment, raw.GetComment())
				...
				for _, member := range *raw.Get().Members {
