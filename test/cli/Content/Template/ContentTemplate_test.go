...
func Test_ContentTemplate_Download_Cleanup(t *testing.T) {
	Test := cliTest.Test{
		Args: []string{"-i", "delete", "file", test.FirstNode, test.GuestStorage, string(proxmox.ContentType_Template), test.DownloadedLXCTemplate},
	}
	Test.StandardTest(t)
}

func Test_ContentTemplate_Existence_Removed_0(t *testing.T) {
	checkIfTemplateDoesNotExist(t, test.DownloadedLXCTemplate, test.FirstNode, test.GuestStorage)
}

func Test_ContentTemplate_Download(t *testing.T) {
	Test := cliTest.Test{
		Contains: []string{"(" + test.DownloadedLXCTemplate + ")"},
		Args:     []string{"-i", "content", "template", "download", test.FirstNode, test.GuestStorage, test.DownloadedLXCTemplate},
	}
	Test.StandardTest(t)
}
...
func Test_ContentTemplate_List(t *testing.T) {
	Test := cliTest.Test{
		Return: true,
		Args:   []string{"-i", "list", "files", test.FirstNode, test.GuestStorage, string(proxmox.ContentType_Template)},
	}
