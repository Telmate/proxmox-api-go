...
func (config ConfigQemu) CreateVm(vmr *VmRef, client *Client) (err error) {
	...
	params := map[string]interface{}{
		"vmid":        vmr.vmId,
		"name":        config.Name,
		"onboot":      boolPointerHasValue(config.Onboot),
		"startup":     config.Startup,
		"tablet":      boolPointerHasValue(config.Tablet),
		"agent":       config.Agent,
		"ostype":      config.QemuOs,
		"sockets":     config.QemuSockets,
		"cores":       config.QemuCores,
		"cpu":         config.QemuCpu,
		"numa":        boolPointerHasValue(config.QemuNuma),
		"kvm":         boolPointerHasValue(config.QemuKVM),
